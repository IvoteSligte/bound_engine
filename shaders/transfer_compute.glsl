...
layout(binding = 0, rgba16f) uniform restrict readonly image2D accumulatorImageRead;
layout(binding = 1, rgba8) uniform restrict writeonly image2D renderImage; // output

void main() {
    vec4 data = imageLoad(accumulatorImageRead, ivec2(gl_GlobalInvocationID.xy));
